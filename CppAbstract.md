...
Все enum должны начинаться с большой буквы Е, тут в коде этого нет. но так должно быть в анриле
...
односвязнный список  это список где каждый элемент уникальный в области памяти и они связаны друг с другом переходам по адресам. Поэтому что бы в такой список добавить элемент, не нужно переписывать весь список, а досточно в конце списка указать где будет лежать этот элемент

Каждый элемент лежит где то в памяти 

мы идем в элемент, смотрим если он не null, у него есть адрес, значит есть еще элемент и идем по адресу этого элемента 

![[Pasted image 20250504005659.png]]

в такой список очень легко добавить и удалить элемент, путем замены адреса на новый адрес элемента 

![[Pasted image 20250504212958.png]]

Но у односвязнаого списка есть свои проблемы, что бы получить элемент какой то в конце, то надо считать с первого элемента, пока не дойдешь до нужного, поэтому удалять элементы или получать их в конце становится долго. 

![[Pasted image 20250504213539.png]]

![[Pasted image 20250504004817.png]]

### Двухсвязный список или двунаправленный

Двусвязный список имеет в своей ячейку поле, которое указывает на прошлый элемент 

![[Pasted image 20250507162036.png]]

Что бы найти элемент в списке, мы можем начинать искать как сначала, так и с конца. Но медленней всего он работает в середине 
Что бы найти элемент мы может использовать общее количество элементов списке, что бы понимать с какой стороны лучше к нем добраться 

![[Pasted image 20250507162619.png]]

Добавление в двухсвязный список, изменение указателей прошлого элемента на новый адрес и в новый адрес на следующий адрес 

![[Pasted image 20250507185806.png]]

Добавление в начало списка 

![[Pasted image 20250508231353.png]]

Удаление первого элемента 

![[Pasted image 20250508231445.png]]

Удаление последнего элемента 

![[Pasted image 20250508231546.png]]

Добавление последнего элемента 

![[Pasted image 20250508231618.png]]

### Бинарное дерево 

![[Pasted image 20250508232226.png]]

Самый первый элемент называется корнем, остальные называются листьми 
Каждый элемент имеет только два элемента в себе, такая структура всегда упорядочена  

Логика строится по тому, заходит число и если оно меньше или больше исходного числа, он ставится в левую или правую ветку. Поэтому когда мы делаем поиск по бинарному дереву, заходя с опредленным числом мы может сразу отсечь те данные, которые нам не нужны, так к в той ветке точно не будет нашего числа. 

Это отличает от односвязного и двухсвязного списка, в которых нужно было бы перебирать все числа

![[Pasted image 20250508232935.png]]

Если бы списки были отсортированы, можно было бы использовать бинарный поиск. Иначе он просто не сработает 

плюсы дерева в том, что он всегда отсортирован 

эти числа это индексы, мы можем под них засууть данные. И как ключ по данным под индексом 53 
![[Pasted image 20250508233554.png]]

Поиск данных под номером 53

![[Pasted image 20250508233745.png]]

![[Pasted image 20250508233829.png]]

Является рекурсивной структурой 

это является тоже бинарным деревом и корнем, поэтому для входа и просмотра всех элементов очень удобно использовать рекурсивные алгоритмы 
![[Pasted image 20250508233927.png]]

### СТЕК

Логика и суть стека. Мы можем в него только добавлять или извлекать из него

Последний зашел, первый вышел

![[Pasted image 20250508234145.png]]

![[Pasted image 20250508234239.png]]

Стек можно сравнить с магазином от пистолета, они достаются в с последнего который был заряжен. Из середины мы взять ничего не можем и добавить тоже 

Что бы добраться до дна, мы должны вытащить все элементы сверху

![[Pasted image 20250508234524.png]]

порядок извлечения, вершина это самый последний добавленный элемент


![[Pasted image 20250508234602.png]]

Добавляем мы всегда только с одной стороны и работаем мы только с вершиной, посчитать общее количество или пройтись по элементам в стеке мы не можем 

![[Pasted image 20250508234707.png]]

Для того что реализовать стек. мы может немного урезать односвязный список. При работе с односвязного самая быстрая работа в начале списка.
Стек это динамеческая структура данных, которая подчиняется логике односвязного списка 

![[Pasted image 20250508234922.png]]

### Очередь. Как динамическая структура данных

Главная правило - первый зашел, первый выходит

![[Pasted image 20250508235201.png]]

С очередью мы можем взаимодействовать в начале или в конце. 

Не можем перебрать все элементы, вставить или убрать из середины. 

Можем получить в первом элементе. Добавляется элемент всегда только в конце очереди. Извлекаются элементы только в начале. Просмотр мы можем в начале и в конце

![[Pasted image 20250508235228.png]]

Лучше всего для очереди подходит двухсвязный список. Лучше всего он работает в начале и в конце, хуже в середине 

![[Pasted image 20250508235705.png]]

## Очередь с приоритетом. Кольцевая очередь 

При работе с очередью, мы первый элемент при извлечении вставялем в конец снова в очередь 

![[Pasted image 20250508235938.png]]

![[Pasted image 20250508235949.png]]

Чем меньше число,, тем больше приоритет 

Очередь с приоритетом, в котором они располагаются в значении от его приоритета 

![[Pasted image 20250509000248.png]]

Очередь без приоритета

Исключение из очереди элементов согласно очереди

![[Pasted image 20250509000755.png]]

## Дек, Deque

Двусторонняя очередь 

![[Pasted image 20250509000915.png]]

Добавление и удаление элементов можем с двух сторон.  

![[Pasted image 20250509001224.png]]

В библиотеке stl мы можем итерироваться по деку

Дек в с++. Совокупность массивов, которые связаны между собой

![[Pasted image 20250509001423.png]]

Когда у нас заканчивается элементы, создаются новый массив. Поиск по такому деку будет быстрее по двух связному списку.

![[Pasted image 20250509001847.png]]

## Библиотеки STL: vector 

Это набор стандартных решений для повседневных задач

Так часто используется что является стандартом 

Контейнер с данными

Vector - массив, который очень прокачен. Сам следит за памятью, может исключать включать в конец данные. Имеет преимущества динамического массива в скорости доступа к данным. 

Так как это последовательно расположенные ячейки в памяти. Непрерывная динамический массив. Быстрее чем односвязные списки 
...
нельзя получить доступ к элементам вне массива 

![[Pasted image 20250509154822.png]]

```cpp
    //попытка поймать исключение
    try
    {
        //при использовании метода .at мы точно не можем выйти за границы вектора
        //но он медленный
...
Количество элементов в векторе 4
clear() Количество элементов в векторе 0
...
Имеется так же такая возможность 

```cpp
    vector<int> myVector = {0, 515, 151, 31};
```
...
При добавлении нового элемента к вектору в массиве, в новый массив куда ячейки скопируются не создается 1 доп ячейка, а создается несколько. Общее количество таких ячеек будет capacity 

![[Pasted image 20250509185600.png]]
...
Количество элементов в векторе 8
capacity вектора 100
shrink_to_fit()
capacity вектора 8

Инициализация 5 элементов в векторе, по 11

```cpp
//cpp
    vector<int> myVector(5, 11);
```
...
Позволяет не указывать точный тип данных переменной, а разрешает комплиятору самому догадаться какой тип данных там лежит, в зависимости от того какие данные мы туда присвоим

ключевое слово может ухудшать читабельность кода, так как не говорит о том какие типы данных это изначально 

```cpp
    //в зависимости от чисел которые присваиваем, компилятор поймет что это тип int
    auto a = 10;

    //поймет как тип double
    auto b = 1.241;

    //char 
    auto c = "string";
    
    //если мы захотим просто объявить переменную, то компилятор не поймет какого типа
    //мы должны сразу инициализировать тип данных
    auto d = true;
```

Если где то для объявления переменной нужно написать много текста, можно написать это использовав ключевое слова auto. Тем самым сократив код. 

Но если есть простые типы данных, такие как int, float.. то не нужно 

```cpp
    vector<int> myVector = {11, 35, 31};

    vector<int>::iterator it = myVector.begin();

    //переменная it2 типа auto будет хранить в себе vector<int>::iterator такой тип данных типа итератора 
    //что бы не указывать весь тип данных, а если не указывать namespace std, то запись еще увеличивается 
    //std::vector<int>::iterator it = myVector.begin();
    //поэтому в it2 мы можем просто засунуть myVector.begin();
...
Реализована возможность удаление и вставки в любом месте контейнера 

Это такая динамическая структура данных. особенность которой она работает быстро с удалением и добавлением данных. 

реализацией является двухсвязный список 
